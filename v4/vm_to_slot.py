    ...
    for child in l_elem:
        if child.tag == f"{{{NS['tei']}}}app":
            ...
            for rdg in child.findall("./tei:rdg", NS):
                ...
                for ed in EDITIONS:
                    if wit == ed or wit.endswith(f"-{ed}") or wit.endswith(ed):
                        ...
                        texts[ed] = val
                        if val:
                            # normalize_text may collapse whitespace; runs remain approximate.
                            # For now keep runs only if raw-normalized lengths still match.
            ...
            if vtype == "empty":
                ...
                annotate_span(span)
                segments.append(span)
                # choose one representative run-set if identical text in all
