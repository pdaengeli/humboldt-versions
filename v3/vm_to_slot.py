...
# --- NEW: simple policy knobs -------------------------------------------------
INLINE_SAFE_PAIRS = {
    "ß|ss", "ss|ß",
    "ae|ä", "oe|ö", "ue|ü",
    "Ae|Ä", "Oe|Ö", "Ue|Ü"
}
INLINE_MAX_SINGLE_REPLACE = 2          # chars
INLINE_ADDITIONS_MAX_LEN = 12          # chars, whitespace-trimmed
# -----------------------------------------------------------------------------
...
# --- NEW: display classifier --------------------------------------------------
def classify_display(span: Dict) -> str:
    """
    Decide whether a span should be inline or marginal.
    Rules:
      - originals stay inline (default)
      - additions: inline if short enough, else marginal
      - substitutions: inline only if all char-level ops are small & in safe set
    """
...
def build_segments_from_l(l_elem) -> List[Dict]:
    ...
    # annotate again in case token splits created new spans
    for s in segments:
        if "display" not in s:
            annotate_span(s)
    return segments
...
def build_slots(root) -> Dict:
    ...
    return {
        "meta": {
            ...
            "editions": EDITIONS,
            "generator": "vm-to-slot-inline-marginal",
            "stats": global_stats
        },
        "content": content
    }
